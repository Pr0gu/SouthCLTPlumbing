  ...
  return (
    ...
    >
      <div className="mx-auto flex max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8 h-14">
        ...
        <nav className="hidden items-center gap-8 md:flex" aria-label="Main navigation">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-sm font-semibold tracking-wide text-gray-300 transition-colors hover:text-brand-gold"
            >
              {link.label}
            </a>
          ))}
        ...
        <a
          href="tel:9804054186"
          className="hidden items-center gap-2 rounded-full bg-brand-gold px-5 py-2 text-sm font-bold text-white shadow-lg transition-all hover:bg-brand-gold-dark md:flex"
      ...
      {/* Mobile menu */}
      <div
        className={`fixed inset-0 top-0 z-40 bg-brand-dark transition-transform duration-300 md:hidden ${
